...
- **Pitch Detector** - Check if audio source is available, prepare dispatcher, preprocess audio (noise reduction), Calculate frequency (represents the pitch) using an algorithm. We will also make sure to close audio stream and release buffers after the session is done.
